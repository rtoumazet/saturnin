...
inline ImGui::FileBrowser::FileBrowser(ImGuiFileBrowserFlags flags) :
    width_(700), height_(450), flags_(flags), openFlag_(false), closeFlag_(false), isOpened_(false), ok_(false),
    inputNameBuf_(std::make_unique<std::array<char, INPUT_NAME_BUF_SIZE>>()) {
    if (flags_ & ImGuiFileBrowserFlags_CreateNewDir) newDirNameBuf_ = std::make_unique<std::array<char, INPUT_NAME_BUF_SIZE>>();
...
inline void ImGui::FileBrowser::Display() {
    ...
    for (auto& sec : pwd_) {
        ...
        if (SmallButton(u8StrToStr(sec.u8string()).c_str())) newPwdLastSecIdx = secIdx;
        PopID();
        ++secIdx;
    }
    ...
    if (!typeFilters_.empty()) {
        ...
        if (BeginCombo("##type_filters", typeFilters_[typeFilterIndex_].c_str())) {
            ...
            for (size_t i = 0; i < typeFilters_.size(); ++i) {
                ...
                if (Selectable(typeFilters_[i].c_str(), selected) && !selected) typeFilterIndex_ = static_cast<int>(i);
            }
        }
        PopItemWidth();
    }
...
inline bool ImGui::FileBrowser::SetPwd(const std::filesystem::path& pwd) {
    try {
        SetPwdUncatched(pwd);
        return true;
    } catch (const std::exception& err) { statusStr_ = std::string("last error: ") + err.what(); } catch (...) {
...
inline void ImGui::FileBrowser::SetCurrentTypeFilterIndex(int index) { typeFilterIndex_ = index; }
...
inline std::uint32_t ImGui::FileBrowser::GetDrivesBitMask() {
    ...
    for (int i = 0; i < 26; ++i) {
        ...
        if (type == DRIVE_REMOVABLE || type == DRIVE_FIXED || type == DRIVE_REMOTE) ret |= (1 << i);
    }
    return ret;
}
