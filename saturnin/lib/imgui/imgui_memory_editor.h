...
// - PageUp/PageDown are supported because we use _NoNav. This is a good test scenario for working out idioms of how to mix
...
struct MemoryEditor {
    ...
    void DrawContents(void* mem_data_void, size_t mem_size, size_t base_display_addr = 0x0000) {
        ...
        if (DataEditingAddr != (size_t)-1) {
            ...
            if (ImGui::IsKeyPressed((ImGuiKey)ImGui::GetKeyIndex(ImGuiKey_UpArrow))
                && (ptrdiff_t)DataEditingAddr >= (ptrdiff_t)Cols) {
                data_editing_addr_next = DataEditingAddr - Cols;
            } else if (ImGui::IsKeyPressed((ImGuiKey)ImGui::GetKeyIndex(ImGuiKey_DownArrow))
                       && (ptrdiff_t)DataEditingAddr < (ptrdiff_t)mem_size - Cols) {
                data_editing_addr_next = DataEditingAddr + Cols;
            } else if (ImGui::IsKeyPressed((ImGuiKey)ImGui::GetKeyIndex(ImGuiKey_LeftArrow))
                       && (ptrdiff_t)DataEditingAddr > (ptrdiff_t)0) {
                data_editing_addr_next = DataEditingAddr - 1;
            } else if (ImGui::IsKeyPressed((ImGuiKey)ImGui::GetKeyIndex(ImGuiKey_RightArrow))
