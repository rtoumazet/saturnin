...
INLINE sint MAKE_INT_8(uint value) { return (value & 0x80) ? value | ~0xff : value & 0xff; }
...
INLINE sint MAKE_INT_16(uint value) { return (value & 0x8000) ? value | ~0xffff : value & 0xffff; }
...
INLINE sint MAKE_INT_32(uint value) { return (value & 0x80000000) ? value | ~0xffffffff : value & 0xffffffff; }
...
#define m68ki_set_address_error_trap() \
    if (setjmp(m68ki_address_error_trap)) m68ki_exception_address_error();
#define m68ki_check_address_error(A) \
    if ((A)&1) longjmp(m68ki_address_error_jump, 1);
#else
#define m68ki_set_address_error_trap()
#define m68ki_check_address_error(A)
...
INLINE uint m68ki_get_ea_ix(uint An) {
    ...
    if (!BIT_8(extension)) {
        ...
        if (CPU_TYPE_IS_EC020_PLUS(CPU_TYPE)) Xn <<= (extension >> 9) & 3; /* SCALE */
...
INLINE void m68ki_exception_address_error(void) { /* Not emulated yet */
}
...
void m68ki_exception_interrupt(uint int_level) {
    ...
    if (vector == M68K_INT_ACK_AUTOVECTOR) /* Use the autovectors.  This is the most commonly used implementation */
    ...
    if (new_pc == 0) new_pc = m68ki_read_data_32((EXCEPTION_UNINITIALIZED_INTERRUPT << 2) + REG_VBR);
...
INLINE void m68ki_check_interrupts(void) {
    if (CPU_INT_LEVEL > FLAG_INT_MASK) m68ki_exception_interrupt(CPU_INT_LEVEL >> 8);
}
