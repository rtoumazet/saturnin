...
inline ExpressionError::ExpressionError(const std::string& whatArg, const InP& it) : std::runtime_error(whatArg), pos(it) {}

ExpressionError::ExpressionError(const char* whatArg, const InP& it) : std::runtime_error(whatArg), pos(it) {}

const InP& ExpressionError::Where() const noexcept { return pos; }
...
inline std::pair<InP, InP> GetExpression(const InP& it, const InP& end, const std::string& keyword) {
    ...
    if (curIt == end) throw ExpressionError("Parse error: '" + keyword + "' is not found.", it);
    std::advance(curIt, keyword.length());
    SkipSpaces(curIt, end);
    if (*curIt != '=') throw ExpressionError("'=' is expected.", curIt);
    ++curIt;
    SkipSpaces(curIt, end);
...
inline ExpFuncType ParseValue(InP& it, const InP& end) {
    ...
    if (it != end) {
        if (*it == 'n') {
            ...
            } catch ([[maybe_unused]] ExpressionError& e) {
                // fall throuth
            }
        }
    }
