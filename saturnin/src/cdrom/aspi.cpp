...
bool Aspi::initialize()
{
    ...
    {
        MessageBox(NULL, GetString(L"AspiMessageGA32SINotFound").c_str(), GetString(L"ASPIMessageCaption").c_str(), MB_ICONSTOP);
        return false;
    }
    ...
    switch (byASPIStatus)
    {
        case SS_COMP:
            ...
            break;
        case SS_NO_ADAPTERS:
            MessageBox(NULL, GetString(L"AspiMessageNoSCSIDevice").c_str(), GetString(L"AspiMessageCaption").c_str(), MB_ICONSTOP);
            return false;
        case SS_NO_ASPI:
            MessageBox(NULL, GetString(L"AspiMessageNoAspiManager").c_str(), GetString(L"AspiMessageCaption").c_str(), MB_ICONSTOP);
            return false;
        case SS_ILLEGAL_MODE:
            MessageBox(NULL, GetString(L"AspiMessageNoRealMode").c_str(), GetString(L"AspiMessageCaption").c_str(), MB_ICONSTOP);
            return false;
        case SS_OLD_MANAGER:
            MessageBox(NULL, GetString(L"AspiMessageAspiTooOld").c_str(), GetString(L"AspiMessageCaption").c_str(), MB_ICONSTOP);
            return false;
        default:
            MessageBox(NULL, GetString(L"AspiMessageErrorInitialization").c_str(), GetString(L"AspiMessageCaption").c_str(), MB_ICONSTOP);
            return false;
    }
...
void Aspi::inquiry(SCSI_DriveInfo & di)
{
    vector<int8_t> v(36);
...
string Aspi::readSector(const uint32_t & dwFAD, const int32_t & iNb)
{
    // CDROM fad
