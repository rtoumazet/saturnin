...
struct SmpcRegs {
    ...
    struct Ddr {
        ...
        static constexpr auto ddr_mask = 0x7F;
    ...
    struct Pdr {
        ...
        static constexpr auto test_sw_pos      = PosType(1); // Defines Test switch in ST-V mode (PDR2).
        static constexpr auto serv_sw_pos      = PosType(2); // Defines Service switch in ST-V mode (PDR2).
        static constexpr auto sound_status_pos = PosType(4); // Defines sound status in ST-V mode (PDR2).

        static constexpr auto test_sw      = BitsType(1, test_sw_pos);
        static constexpr auto serv_sw      = BitsType(1, serv_sw_pos);
        static constexpr auto sound_status = BitsType(1, sound_status_pos);

        static constexpr auto pdr_mask = 0x7F;
